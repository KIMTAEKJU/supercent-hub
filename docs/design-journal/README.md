...
이 저널은 작업 중 세션 transcript 를 **NotebookLM** (RAG 지식베이스) 에 아카이브해두고, 각 주제별로 7건의 **쿼리**를 날려 얻은 합성 요약을 파일로 정리한 것입니다.

- **원문**: NotebookLM 아카이브 `supercent-study-001/002/003` (비공개)
- **합성**: NotebookLM RAG 요약
- **편집·구조화·민감정보 제거**: 사람이 진행
- **citation 마커 제거**: `sed` 로 일괄 정리

즉 "Claude 가 막 지어낸 이야기" 가 아니라 **실제 세션 로그를 RAG 합성한 결과**입니다. 원문 접근이 필요하시면 별도 문의 주시면 제공 가능합니다.
